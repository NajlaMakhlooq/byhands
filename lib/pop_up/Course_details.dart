...
class _CourseDetailPageState extends State<CourseDetailPage> {
  ...
  Future<void> fetchCourses() async {
    try {
      final response =
          await supabase.from('Courses').select().eq('Name', widget.courseName);

      setState(() {
        CourseDetails = (response as List<dynamic>?)
                ?.map((e) => {
                ...
                Padding(
                  ...
                  child: Container(
                    ...
                    child: Padding(
                      ...
                      child: Text(
                        "cdd.description ${course['Description'].toString()}".tr(),
                      ),
                    ),
                  ),
                ),
      ...
      if (existingEntry != null) {
        ...
        showDialog(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              title: Text("ccd.AlreadyLiked".tr()),
              content: Form(
                key: formfield,
                child: SingleChildScrollView(
                  child: Text("ccd.AlreadyLikedMessage".tr()),
                ),
              ),
              actions: [
                TextButton(
                  child: Text("ccd.ok".tr()),
                  onPressed: () {
